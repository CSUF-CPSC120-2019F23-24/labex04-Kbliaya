...
int main()
{
  ...
  char my_char1 = 'J';
  short my_short1 = 74;
  std::cout << "The ASCII value for " << my_char1 << " is " << my_short1 << '\n';
  std::cout << static_cast<short>(74) << '\n'; // blank line to separate output from the different exercises
  std::cout << '\n';
  ...
  std::cout << '\n'; // blank line to separate output from the different exercises
  //The character is no longer alphabetic because it reached the max value and wrapped
  ...
  std::cout << "Int (" << sizeof(my_short2) << " bytes): "
    ...
    my_int2 = my_int2 = my_int2 + 1;
    std::cout << ", after adding 1: " << std::dec << my_int2
    << std::hex << " (0x" << my_int2 << ")\n";
  std::cout << '\n'; // blank line to separate output from the different exercises
  //my_short2 value became negative because int has more data compared to short so
  //the value doesn't need to wrap around to the negatives until later.
